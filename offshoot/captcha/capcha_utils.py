...
def filter(img):
    ...
    path = r"C:\Users\Administrator\PycharmProjects\captcha\filter\{}filter.jpg".format(nowTime)
    img.save(path)
    return path
...
def cut_image(img):
    ...
    for i in [cropped, cropped2, cropped3]:
        ...
        if len(text) > text_len and '验证码' not in text and '收款人' not in text:
            text_len = len(text)
            target_image = i
...
def process2(number, source_img):
    path = filter(source_img)
    i = cv2.imread(path)
    # i = cv2.resize(i, (573, 97))
    img = cut_image(i)
    lowerb = (0, 0, 116)
    upperb = (255, 255, 255)
    # 根据hsv阈值 进行二值化
    ...
    print("1---" + str(len(edge_array)))
    print(edge_array)

    # os.remove(path)
    # 框的数量少于number一律视为yellow
    # if len(edge_array) < len(number):
    edge_array, canvas = yellow_number_process(source_img)
    # edge_array, canvas = blue_number_process(source_img)
    print("2---" + str(len(edge_array)))
    ...
    nowTime = datetime.datetime.now().strftime('%Y%m%d_%H_%M_%S')
    cv2.imwrite(r"C:\Users\Administrator\PycharmProjects\captcha\res\2{}.jpg".format(nowTime), canvas)
...
def yellow_number_process(source_img):
    low_yellow = np.array([0, 0, 70])
    high_yellow = np.array([255, 75, 255])
...
def blue_number_process(source_img):
    low_yellow = np.array([0, 0, 70])
    high_yellow = np.array([255, 49, 255])
...
def save_cut_image(edge_array, canvas):
    for n, N in enumerate(edge_array):
        x, y, w, h, index = N
        cv2.rectangle(canvas, pt1=(x, y), pt2=(x + w, y + h), color=(0, 255, 255), thickness=1)
...
def show_cut_image(edge_array, canvas):
    for n, N in enumerate(edge_array):
        x, y, w, h = N
        cv2.rectangle(canvas, pt1=(x, y), pt2=(x + w, y + h), color=(0, 255, 255), thickness=1)
    cv2.imshow('ada', canvas)
    cv2.waitKey(0)


def check_box(edge_array):
