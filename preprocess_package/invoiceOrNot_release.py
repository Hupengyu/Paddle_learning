...
if __name__ == '__main__':
    ...
    if type(imgs_list) == np.ndarray:
        invoices_num = detect_image_counts(imgs_list)   # 查看是几张发票
        # if invoices_num > 1:
        cut_images_save(imgs_list, crops_save_path)     # 无论是一张还是两张都放进去
    else:
